  ...
  try {
    ...
    try {
      ...
      console.log("OPTIONS request result:", {
        ...
        headers: [...optionsResponse.headers.entries()]
          .map((e) => `${e[0]}: ${e[1]}`)
          .join(", "),
      });
    } catch (optionsError) {
    ...
    console.log(
      ...
      [...response.headers.entries()].map((e) => `${e[0]}: ${e[1]}`).join(", ")
    );
...
async function handleGoogleAuth() {
  try {
    ...
    console.log(
      ...
      accessToken ? "Token received" : "No token found"
    );
...
async function handleApiRequest({
  ...
  try {
    ...
    if (method !== "GET" && method !== "HEAD") {
      console.log(
        `Testing CORS with OPTIONS request for ${method} ${endpoint}...`
      );
      try {
        ...
        console.error(
          `OPTIONS preflight failed for ${endpoint}:`,
          optionsError
        );
      }
    }
    ...
    if (
      ...
      authState.user &&
      authState.user.id &&
      (endpoint.includes("/api/ai-chat") ||
        endpoint.includes("/api/ai-chat-sessions"))
    ) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "UPLOAD_FILE") {
    ...
    if (!hostId) {
      console.error(
        "File upload error: No host ID provided and user not authenticated"
      );
      sendResponse({
        success: false,
    ...
    try {
      ...
      console.log("Created blob from file data:", {
        type: mimeString,
        size: blob.size,
      });
